...
def _augment(img, mode):
    """Apply geometric augmentation. mode 0-7."""
    if mode == 0:
        return img
    elif mode == 1:
        return img.flip(-1)
...
class DATModel(SRModel):
    ...
    def _tta_forward(self, lq):
        outputs = []
        for mode in range(8):
            aug = _augment(lq, mode)
            out = self._forward(aug)
            outputs.append(_inverse_augment(out, mode))
        return torch.stack(outputs, dim=0).mean(dim=0)

    def test(self):
        self.use_chop = self.opt['val'].get('use_chop', False)
        self.use_tta  = self.opt['val'].get('use_tta',  False)

        if not self.use_chop:
            if self.use_tta:
                self.output = self._tta_forward(self.lq)
